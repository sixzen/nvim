        ...
        utils.surround(surround_delimeter, "mantle", {
          provider = function()
            ...
            for _, server in pairs(vim.lsp.get_clients { bufnr = 0 }) do
              if server.name ~= "null-ls" and server.name ~= "GitHub Copilot" and server.name ~= "copilot" and server.name ~= "emmet_ls" then
                table.insert(names, server.name)
              end
              if server.name == "GitHub Copilot" or server.name == "copilot" then
                copilot = true
              end
            end
